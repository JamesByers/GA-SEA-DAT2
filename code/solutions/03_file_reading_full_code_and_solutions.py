...
with open('/Users/jim_byers/Documents/GA/GA_Data_Science_course/SEA-DAT1/data/airlines.csv', mode='rU') as f:    
...
# solution to part 2 and 3 all at once
airlines = []
starred = []
for row in data:
    if row[0][-1] == '*':
...
# Note: the solutions to part 4 assume that the solution to Part 1 has been run to create the list “incidents”
incidents = [round((int(row[2]) + int(row[5])) / float(30), 2) for row in data]

# Note: the solutions to part 4 assume that the solution to Part 2 has been run to create the list “airlines“
airlines = []
for row in data:
    if row[0][-1] == '*':
        airlines.append(row[0][:-1])
...
# Part 4 solution 1:
airline_incidents = {airlines[n]:incidents[n] for n in range(0,len(airlines))}
